...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://ghostcode.ai/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://ghostcode.ai/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://ghostcode.ai/config.json" }, null, 2))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://ghostcode.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://ghostcode.ai/config.json"
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["GHOSTCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("GHOSTCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.OPENCODE_PERMISSION) {
          try {
            ...
            yield* Effect.logWarning("GHOSTCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
