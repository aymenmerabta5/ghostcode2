...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      if (Array.isArray(apiKeys) && apiKeys.length > 0) {
        const firstKey = apiKeys[0] as any
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // load per-provider apiKeys.json for rotation pools
        {
          const apiKeysBaseDir = path.join(Global.Path.data, "providers")
          for (const [id] of Object.entries(database)) {
            const providerID = ProviderV2.ID.make(id)
            if (disabled.has(providerID)) continue
            const existing = providers[providerID]?.options?.["apiKeys"]
            if (Array.isArray(existing) && existing.length > 0) continue
            const apiKeysFile = path.join(apiKeysBaseDir, id, "apiKeys.json")
            const loaded = yield* Effect.promise(async () => {
              try {
                const data = JSON.parse(await Bun.file(apiKeysFile).text())
                if (Array.isArray(data) && data.length > 0) return data
            ...
            const patch: Partial<Info> = providers[providerID]
              ? { options: { apiKeys: loaded } }
              : { source: "custom", options: { apiKeys: loaded } }
        ...
        for (const [id, provider] of Object.entries(providers)) {
          const providerID = ProviderV2.ID.make(id)
          const apiKeys = provider.options?.["apiKeys"]
          if (!Array.isArray(apiKeys) || apiKeys.length === 0) continue
          const cooldownMs =
            typeof provider.options?.["cooldownMs"] === "number" ? provider.options["cooldownMs"] : undefined
          rotator.init(providerID, apiKeys as KeyRotator.KeyEntry[], cooldownMs)
        }
      ...
      try {
        ...
        if (keyOverride) {
          if (typeof keyOverride.entry === "string") {
            ...
            if ("apiKey" in keyOverride.entry && typeof keyOverride.entry.apiKey === "string") {
              options["apiKey"] = keyOverride.entry.apiKey
            }
            if ("accountId" in keyOverride.entry) {
              options["_rotatedAccountId"] = (keyOverride.entry as any).accountId
            }
            if ("apiUrl" in keyOverride.entry) {
              options["_rotatedApiUrl"] = (keyOverride.entry as any).apiUrl
              options["credentials"] = {
                apiKey: (keyOverride.entry as any).apiKey,
                apiUrl: (keyOverride.entry as any).apiUrl,
              }
            }
          }
