...
const ghostcodeKey = (name: string) => name.replace(/^OPENCODE_/, "GHOSTCODE_")
const bool = (name: string) =>
  Config.boolean(ghostcodeKey(name)).pipe(
    Config.orElse(() => Config.boolean(name)),
    Config.withDefault(false),
  )
const positiveInteger = (name: string) =>
  Config.number(ghostcodeKey(name)).pipe(
...
const enabledByExperimental = (name: string) =>
  Config.all({
    experimental,
    enabled: Config.boolean(ghostcodeKey(name)).pipe(
      Config.orElse(() => Config.boolean(name)),
      Config.option,
    ),
  }).pipe(
...
export class Service extends ConfigService.Service<Service>()("@opencode/RuntimeFlags", {
  ...
  client: Config.string("GHOSTCODE_CLIENT").pipe(
