      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "step-finish": {
            ...
            yield* Effect.logInfo("processor step-finish", {
          ...
          yield* Effect.gen(function* () {
            ...
            Effect.retry(
              SessionRetry.policy({
                ...
                parse,
                set: (info) =>
                  // Silent rotation: while keys remain available, do NOT surface rate-limit /
                  // exhaustion errors to the TUI — just rotate and retry. Only once every key is
                  // cooled/exhausted do we show the retry status. Non-rate-limit errors (e.g. the
                  // Go upsell action) are always shown.
                  info.isRateLimit
                    ? Effect.gen(function* () {
                        const { available } = yield* provider.getRotation(input.model.providerID)
                ...
                onRateLimited: (info) =>
                  keyIndex.current !== undefined
                    ? provider.markRateLimited(input.model.providerID, keyIndex.current, {
                        retryAfterMs: info?.retryAfterMs,
                      })
                    : Effect.void,
                onKeyExhausted: () =>
                  keyIndex.current !== undefined
                    ? provider.markRateLimited(input.model.providerID, keyIndex.current, { exhausted: true })
                    : Effect.void,
                canRotateKey: () =>
                  Effect.gen(function* () {
                    return (yield* provider.getRotation(input.model.providerID)).available > 0
                  }),
                delay: ({ isRateLimit }) =>
                  Effect.gen(function* () {
                    if (!isRateLimit) return Duration.millis(SessionRetry.RETRY_FIXED_DELAY)
                    const { available, waitMs } = yield* provider.getRotation(input.model.providerID)
                    if (available > 0) return Duration.millis(0)
