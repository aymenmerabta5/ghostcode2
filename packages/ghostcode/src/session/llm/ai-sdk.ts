...
export function toLLMEvents(
  ...
  switch (event.type) {
    ...
    case "finish-step":
      // The AI SDK defaults the step finish reason to "other" and only assigns a real
      // value when a chunk carries finish_reason. A finish-step still at "other" means
      // the provider closed the stream without signalling completion (a dropped or
      // truncated connection — common with Cloudflare Workers AI under load). That is
      // not a clean completion: raise it as a stream error so the retry policy rotates
      // keys / retries instead of the session going idle mid-task ("silent stop").
      if (event.finishReason === "other") {
        return Effect.fail(new Error("stream ended without finish"))
      }
      // A step that reports "stop" but produced zero text deltas and zero tool calls
      // is a degenerate response — the provider cut the stream mid-generation (key
      // exhaustion / rate-limit) yet still sent a finish_reason=stop chunk. Without
      // this guard the loop sees finish=stop, no tool calls, no text and exits
      // silently, leaving the user with no output ("silent stop").
      if (state.stepStarted && !state.stepHasContent && event.finishReason === "stop") {
        return Effect.fail(new Error("stream ended without content"))
      }
      return Effect.gen(function* () {
        const reason = finishReason(event.finishReason)
        yield* Effect.logInfo("adapter finish-step", { raw: event.finishReason, normalized: reason })
