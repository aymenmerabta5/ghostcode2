...
describe("session.llm.ai-sdk adapter", () => {
  ...
  test("raises error on synthetic 'other' finishReason (dropped stream)", async () => {
    ...
    if (Exit.isFailure(exit)) {
      ...
      expect((error as Error).message).toBe("stream ended without finish")
    }
  })

  test("raises error on 'stop' finishStep with no text or tool content (silent stop)", async () => {
    const state = LLMAISDK.adapterState()
    const exit = await Effect.runPromiseExit(
      Effect.gen(function* () {
        yield* LLMAISDK.toLLMEvents(state, { type: "start-step", request: {}, warnings: [] })
        yield* LLMAISDK.toLLMEvents(state, {
          type: "reasoning-start",
          id: "reasoning-1",
          providerMetadata: undefined,
        })
        yield* LLMAISDK.toLLMEvents(state, uncheckedAdapterEvent({ type: "reasoning-delta", text: "thinking..." }))
        yield* LLMAISDK.toLLMEvents(state, uncheckedAdapterEvent({ type: "reasoning-end" }))
        yield* LLMAISDK.toLLMEvents(state, {
          type: "finish-step",
          response: { id: "r1", timestamp: new Date(0), modelId: "gpt-test" },
          finishReason: "stop",
    ...
    if (Exit.isFailure(exit)) {
      const error = Cause.squash(exit.cause)
      expect(error).toBeInstanceOf(Error)
      expect((error as Error).message).toBe("stream ended without content")
    }
  })

  test("does not raise error on 'stop' finishStep when text deltas were emitted", async () => {
    const state = LLMAISDK.adapterState()
    const events = await Effect.runPromise(
      Effect.gen(function* () {
        const result = yield* Effect.forEach(
          [
            { type: "start-step", request: {}, warnings: [] } as AISDKAdapterEvent,
            uncheckedAdapterEvent({ type: "text-delta", text: "Hello" }),
            {
              type: "finish-step",
              response: { id: "r1", timestamp: new Date(0), modelId: "gpt-test" },
              finishReason: "stop",
