...
describe("session.message-v2.fromError", () => {
  ...
  test("detects context overflow from Cloudflare Workers AI token limit error", () => {
    const error = new APICallError({
      message: "Bad Request",
      url: "https://api.cloudflare.com/client/v4/accounts/abc/ai/run/@cf/model",
      requestBodyValues: {},
      statusCode: 400,
      responseHeaders: { "content-type": "application/json" },
      responseBody: JSON.stringify({
        errors: [
          {
            message:
              'AiError: AiError: {"object":"error","message":"Requested token count exceeds the model\'s maximum context length of 256000 tokens. You requested a total of 259192 tokens: 227192 tokens from the input messages and 32000 tokens for the completion. Please reduce the number of tokens in the input messages or the completion to fit within the limit.","type":"BadRequestError","param":null,"code":400}',
            code: 8007,
          },
        ],
        success: false,
